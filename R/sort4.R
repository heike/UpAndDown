...
sort4 <- function(data, gvar, weight=1, v1, v2) {

  FD <- data.frame(data)

  sL <- c("orig", "base", "perc", "abs")
  nL <- c("Original order", "Base", "Percentage change", "Absolute change")
  xd <- list()
  xw <- list()
  xp <- list()
  xpl <- list()
  ...
  a5 <- gridExtra::arrangeGrob(xp[[1]], xp[[3]], xp[[2]], xp[[4]], nrow=2)

  b5 <- gridExtra::arrangeGrob(xpl[[1]], xpl[[3]], xpl[[2]], xpl[[4]], nrow=2)
