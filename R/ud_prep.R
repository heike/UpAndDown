...
if(getRversion() >= "2.15.1")  utils::globalVariables(c("base", "c1F",
                                                        "c1base", "c1perc", "c1s", "c1v1", "c1v2", "c2F", "c2base", "c2perc", "c2s",
...
ud_prep <- function(data, weight=1, v1, v2, levs, sortLev) {
  ...
  lo <- length(sortLev)
  if (lo < lc)
    stop("Not enough sortings have been specified.  There should be as many as the number of levels variables.", call.=FALSE)
  if (lo > lc)
    stop("Too many sortings have been specified.  There should be as many as the number of grouping variables.", call.=FALSE)
  sortings <- c("orig", "base", "perc", "abs")
  ...
  if (lc==1) {
    INX <- INX %>% group_by(c1F) %>%  mutate(nID=as.integer(c1F), c1v1=sum(wt*v1),
                                             c1v2=sum(wt*v2), c1base=c1v1, c1orig=mean(nID), c1abs=c1v2-c1v1, c1perc=100*(c1v2/c1v1-1)) %>% ungroup()
    INX <- fsort(INX, sortLev[1])
    return(list(levs=levs, sortLev=sortLev, hx=NULL, lgv=lgv, data = INX))
  }
  ...
  if (Nesting=="allNested") {
    ...
    if (lc==2) {
      ...
      INX <- INX %>% group_by(c1F, c2F) %>% mutate(n2ID=as.integer(c2F), c2v1=sum(wt*v1),
                                                   c2v2=sum(wt*v2), c2orig=mean(n2ID), c2base=c2v1, c2abs=c2v2-c2v1, c2perc=100*(c2v2/c2v1-1)) %>% ungroup()

      # Order by top level and then by lower level
      INX <- fsort(INX, sortLev[1])
      INX <- fsort2(INX, sortLev[2])
      return(list(levs=levs, sortLev=sortLev, hx=hx, lgv=lgv, data = INX))
    }
    ...
    if (lc==3) {
      ...
      INX <- INX %>% group_by(c1F, c2F, c3F) %>% mutate(n3ID=as.integer(c2F), c3v1=sum(wt*v1),
                                                        c3v2=sum(wt*v2), c3orig=mean(n3ID), c3base=c3v1, c3abs=c3v2-c3v1, c3perc=100*(c3v2/c3v1-1)) %>% ungroup()
      INX <- fsort3(INX, sortLev[3])
  ...
  if (Nesting=="noneNested"){
    ...
    if (lc==2) {
      ...
      INX <- INX %>% group_by(c1F) %>% mutate(nID=as.integer(c1F), c1v1=sum(wt*v1), c1v2=sum(wt*v2),
                                              c1orig=mean(nID), c1base=c1v1, c1abs=c1v2-c1v1, c1perc=100*(c1v2/c1v1-1)) %>% ungroup()
      INX <- INX %>% group_by(c2F) %>% mutate(n2ID=as.integer(c2F), c2v1=sum(wt*v1), c2v2=sum(wt*v2),
                                              c2orig=mean(n2ID), c2base=c2v1, c2abs=c2v2-c2v1, c2perc=100*(c2v2/c2v1-1)) %>% ungroup()
    ...
    if (lc==3) {
      ...
      INX <- INX %>% group_by(c3F) %>% mutate(n3ID=as.integer(c3F), c3v1=sum(wt*v1), c3v2=sum(wt*v2),
                                              c3orig=mean(n3ID), c3base=c3v1, c3abs=c3v2-c3v1, c3perc=100*(c3v2/c3v1-1)) %>% ungroup()
